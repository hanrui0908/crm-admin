var get_sub_lore_index = function() {//科目
    var param = "{}";
    var optionHtml = "";
    var url = "http://192.168.0.137:8080/yc-crm/sub/search/" + param;
    var msg = dataM(url);
    if(msg!=""){
...
function get_sub() {//科目
    var optionHtml = "";
    var url = "http://192.168.0.137:8080/yc-crm/sub/search";
    var obj={};
    obj.param='{}';
...
var get_year_lore_index = function(){//年份
	var param = "{}";
    var optionHtml = "";
    var url = "http://192.168.0.137:8080/yc-crm/year/search/" + param;
    var msg = dataM(url);
    if(msg!=""){
    	var optionDom = msg.result.result;
	    for(var i =0;i<optionDom.length;i++){
...
function get_year() {//科目
    var optionHtml = "";
    var url = "http://192.168.0.137:8080/yc-crm/year/search";
    var obj={};
    obj.param='{}';
    var msg = dataM(url,obj);
    if(msg!=""){
...
function item_add_submit() {
   ...
   console.log(param);
   /* $.ajax({
        url: "http://192.168.0.137:8080/yc-crm/item/add?param='" + param + "'",
        type: "post",
        dataType: "json",
        success: function (msg) {
            if (msg.status == 0) {
